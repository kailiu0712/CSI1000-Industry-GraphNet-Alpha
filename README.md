...
The table below is gross of transaction costs, which is the view that
isolates how well the ranking works. Costs are modelled in full and reported
in [Transaction costs](#transaction-costs) further down.
...
| Long-short annualised return | 1062.4% | 352.9% |
| Long-only annualised return | 209.8% | 80.5% |
...
One convention to keep straight: the curves are **additive** (`cumsum`,
matching the parent framework's charts) while the annualised returns in the
table are **geometric**. The curves show the shape of a track record; they are
not meant to be read off at the right-hand edge.
...
The decile bar shows the same thing as levels. The spread is monotone across
all ten buckets rather than driven by one extreme (Spearman monotonicity
1.000): Q10 earns +0.247% a day, Q1 loses -0.358%, and every step in between
moves the right way.

![Decile returns, out-of-sample](docs/figures/GNN_IC4Net_test_close_t1_decile_bar.png)

Worth noting what the market did over this window. The equal-weighted CSI 1000
returned **-4.9% annualised** at a Sharpe of -0.07 — flat to slightly down —
so the long-only top decile's 80.5% and Sharpe of 2.43 are not market drift in
disguise. The excess-of-benchmark Sharpe is 7.78.

The RankIC series shows where the stability comes from: IC is positive on
**86% of days** rather than being carried by a few large ones.

![RankIC over time](docs/figures/GNN_IC4Net_test_close_t1_ic.png)

### Transaction costs

The figures above are gross. Costs are modelled in full: stamp duty 5 bps
sell-side only (10 bps before China halved it on 2023-08-28, which falls
inside the test window), commission 2.5 bps and transfer fee 0.1 bps both
sides, slippage 5 bps a side, plus 800 bps annualised borrow on the short leg
— about a 20 bps round trip, charged on each day's actual bucket turnover. At
78% daily turnover that is a **~17 bps daily drag**, and it is the binding
constraint on this strategy, not the signal. Out of sample the long-short leg
goes from a Sharpe of 10.25 and 352.9% annualised gross to **4.41 and 90.6%
net**; the long-only top decile from 2.43 and 80.5% to **0.74 and 17.0%**,
against a benchmark that returned -4.9%, for a net excess Sharpe of **2.52**.
Decile monotonicity stays 1.000 after costs, so the ranking itself is
undamaged — what costs take is the level, not the ordering. Both legs staying
positive is the non-obvious part; a 78%-turnover signal usually does not
survive A-share frictions at all. The natural next step is a turnover or
holding-period penalty in the training objective, which this version does not
have. `--no-costs`, `--slippage-bps` and `--commission-bps` adjust the
assumptions; the full per-component table is in
[`docs/GNN_IC4Net_transaction_costs.csv`](docs/GNN_IC4Net_transaction_costs.csv)
and the net figures for every window are in the run's metrics file.
...
to `docs/figures/` — three per (window, holding period) plus the Barra
